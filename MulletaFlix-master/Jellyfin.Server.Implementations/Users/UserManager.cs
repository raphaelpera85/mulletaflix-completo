        ...
        private static async Task EnsureUserSchemaTablesAsync(UsersDbContext dbContext)
        {
            ...
            if (supportsMySql)
            {
                ...
                [
                    ...
                    """,
                    """
                    CREATE TABLE IF NOT EXISTS `PlaybackReport` (
                        `Id` bigint NOT NULL AUTO_INCREMENT,
                        `UserId` char(36) CHARACTER SET ascii COLLATE ascii_general_ci NOT NULL,
                        `ItemId` char(36) CHARACTER SET ascii COLLATE ascii_general_ci NOT NULL,
                        `ItemName` varchar(512) NULL,
                        `ItemType` varchar(64) NULL,
                        `SeriesName` varchar(512) NULL,
                        ...
                        `PlaySessionId` varchar(128) NULL,
                        ...
                        `ItemRuntimeTicks` bigint NULL,
                        `CompletionPercentage` double NULL,
                        `PlayedToCompletion` tinyint(1) NOT NULL,
                        `WasTranscoded` tinyint(1) NOT NULL,
                        `VideoCodec` varchar(64) NULL,
                        `AudioCodec` varchar(64) NULL,
                        `Container` varchar(32) NULL,
                        `Bitrate` bigint NULL,
                        `Width` int NULL,
                        `Height` int NULL,
                        `Protocol` varchar(32) NULL,
                        `PlayMethod` varchar(32) NULL,
                        `RemoteEndPoint` varchar(64) NULL,
                        `IsLocal` tinyint(1) NOT NULL,
                        `LibraryId` char(36) CHARACTER SET ascii COLLATE ascii_general_ci NULL,
                        `LibraryName` varchar(256) NULL,
                        `ErrorMessage` varchar(1024) NULL,
                        `DateCreated` datetime(6) NOT NULL,
                        `LogSeverity` int NOT NULL,
                        `RowVersion` int unsigned NOT NULL,
                        CONSTRAINT `PK_PlaybackReport` PRIMARY KEY (`Id`),
                        INDEX `IX_PlaybackReport_DateCreated` (`DateCreated`),
                        INDEX `IX_PlaybackReport_DeviceId` (`DeviceId`),
                        INDEX `IX_PlaybackReport_DeviceId_DateCreated` (`DeviceId`, `DateCreated`),
                        INDEX `IX_PlaybackReport_ItemId` (`ItemId`),
                        INDEX `IX_PlaybackReport_ItemId_DateCreated` (`ItemId`, `DateCreated`),
                        INDEX `IX_PlaybackReport_PlaySessionId` (`PlaySessionId`),
                        INDEX `IX_PlaybackReport_UserId` (`UserId`),
                        INDEX `IX_PlaybackReport_UserId_DateCreated` (`UserId`, `DateCreated`)
