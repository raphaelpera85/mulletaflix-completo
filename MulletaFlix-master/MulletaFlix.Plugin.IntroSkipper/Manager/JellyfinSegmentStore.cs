...
internal sealed partial class JellyfinSegmentStore(
    IDbContextFactory<MulletaFlixDbContext> contextFactory,
    ILogger<JellyfinSegmentStore> logger) : IJellyfinSegmentStore
{
    ...
    private static IQueryable<MediaSegment> OwnSegments(MulletaFlixDbContext db, Guid itemId)
        => db.MediaSegments.Where(segment => segment.ItemId == itemId && segment.SegmentProviderId == ProviderId);

    private async Task SaveExactlyAsync(MulletaFlixDbContext db, Guid itemId, string operation, int expectedWrites, CancellationToken cancellationToken)
    {
    ...
    private async Task RunWriteTransactionAsync(MulletaFlixDbContext db, Func<Task> writes, CancellationToken cancellationToken)
    {
