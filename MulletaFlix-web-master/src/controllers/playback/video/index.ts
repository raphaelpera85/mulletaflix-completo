...
export default function (this: { touchHelper?: { destroy(): void } }, view: HTMLElement): void {
    ...
    function updateRecordingButton(item: ItemDto | null) {
    ...
    function updateDisplayItem(itemInfo: { originalItem: ItemDto; displayItem?: ItemDto }) {
        ...
        if (trickplayResolutions) {
            ...
            for (const [, info] of Object.entries(trickplayResolutions) as Array<[string, any]>) {
                if (!bestWidth
    ...
    function getDisplayTimeWithoutAmPm(date: Date, showSeconds?: boolean) {
        if (showSeconds) {
    ...
    function setDisplayTime(elem: HTMLElement, date: string | null) {
        let html;

        if (date) {
            const parsedDate = datetime.parseISO8601Date(date);
    ...
    function updateNowPlayingInfo(player: any, state: any) {
    ...
    function setTitle(item: ItemDto, parentName: string | undefined) {
        ...
        if (item.Type == 'Movie' && item.ProductionYear) {
            title += ` (${item.ProductionYear.toString()})`;
        } else if (item.PremiereDate) {
            try {
                const year = datetime.parseISO8601Date(item.PremiereDate).getFullYear().toString();
                title += ` (${year})`;
            } catch (e) {
                console.error(e);
            }
    ...
    function showMainOsdControls(focusElement: HTMLElement | null) {
    ...
    function hideMainOsdControls() {
        if (currentVisibleMenu === 'osd') {
            ...
            if (activeElement && (osdBottomElement.contains(activeElement)
    ...
    function onInputCommand(e: Event) {
    ...
    function onPlayPauseStateChanged(this: any) {
        if (isEnabled) {
    ...
    function onPlaybackStopped(this: any, e: any, state: any) {
    ...
    function onTimeUpdate(this: any) {
    ...
    function showComingUpNextIfNeeded(player: any, currentItem: any, currentTimeTicks: any, runtimeTicks: any) {
    ...
    function updatePlayerStateInternal(event: any, player: any, state: any) {
    ...
    function getDisplayPercentByTimeOfDay(programStartDateMs: number, programRuntimeMs: number, currentTimeMs: number) {
        return (currentTimeMs - programStartDateMs) / programRuntimeMs * 100;
    }

    function updateTimeDisplay(positionTicks: any, runtimeTicks: any, playbackStartTimeTicks: any, playbackRate: any, bufferedRanges: any) {
        if (enableProgressByTimeOfDay) {
            if (nowPlayingPositionSlider && !nowPlayingPositionSlider.dragging) {
    ...
    function updatePlayerVolumeState(player: ReturnType<typeof playbackManager.getCurrentPlayer>, isMuted: boolean | undefined, volumeLevel: number | undefined) {
    ...
    function updateTimeText(elem: HTMLElement, ticks: number | null, divider?: boolean) {
        if (ticks == null) {
            elem.innerHTML = '';
            return;
        }
    ...
    function showSecondarySubtitlesMenu(actionsheet: any, positionTo: any) {
        ...
        const menuItems = streams.map(function (stream: any) {
            const opt: any = {
                name: stream.DisplayTitle,
                id: stream.Index
            };
    ...
    function showSubtitleTrackSelection(this: any) {
        const player = currentPlayer;
    ...
    function onClickCapture(e: Event) {
        ...
        if (!(e.target as Element)?.contains(clickedElement)) {
            e.preventDefault();
            e.stopPropagation();
            return false;
        }
    ...
    function updateTrickplayBubbleHtml(apiClient: any, trickplayInfo: any, item: any, mediaSourceId: any, bubble: any, positionTicks: any) {
    ...
    function getImgUrl(item: any, chapter: any, index: any, maxWidth: any, apiClient: any) {
        if (chapter.ImageTag) {
    ...
    function getChapterBubbleHtml(apiClient: any, item: any, chapters: any, positionTicks: any) {
        let chapter;
        let index = -1;
    ...
    let playPauseClickTimeout: ReturnType<typeof setTimeout> | null = null;
    ...
    function enableStopOnBack(enabled: boolean) {
        view.removeEventListener('viewbeforehide', onViewHideStopPlayback);
    ...
    function updatePlaybackRate(player: any) {
    ...
    let comingUpNextDisplayed: any;
    let currentUpNextDialog: any;
    let isEnabled: boolean | any;
    let currentItem: any;
    let recordingButtonManager: any;
    let enableProgressByTimeOfDay: any;
    let currentVisibleMenu: any;
    ...
    nowPlayingVolumeSlider.addEventListener('input', (e: any) => {
        playbackManager.setVolume(e.target.value, currentPlayer);
    });

    nowPlayingPositionSlider.addEventListener('change', function (this: HTMLInputElement) {
        const player = currentPlayer;
    ...
    nowPlayingPositionSlider.addEventListener('keydown', function (e: any) {
    ...
    nowPlayingPositionSlider.updateBubbleHtml = function(bubble: any, value: any) {
        showOsd();
    ...
    nowPlayingPositionSlider.getBubbleHtml = function (value: any) {
        showOsd();
        if (enableProgressByTimeOfDay) {
            if (programStartDateMs && programEndDateMs) {
                ...
                return '<h1 class="sliderBubbleText">' + getDisplayTimeWithoutAmPm(new Date(ms), true) + '</h1>';
            }
    ...
    nowPlayingPositionSlider.getMarkerInfo = function () {
        ...
        return currentItem?.Chapters?.map((currentChapter: any) => ({
