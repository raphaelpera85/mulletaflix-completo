...
export class HtmlVideoPlayer {
    ...
    getTextTracks() {
        ...
        if (videoElement) {
            return Array.from(videoElement.textTracks)
                .filter(function (trackElement: any) {
    ...
    _setSubtitleOffset(offset) {
        ...
        if (this.#currentAssRenderer) {
            ...
            if (trackElements?.length > 0) {
                trackElements.forEach((trackElement: any, index: any) => {
    ...
    setTrackEventsSubtitleOffset(trackEvents, offsetValue, currentTrackIndex) {
        if (Array.isArray(trackEvents)) {
            ...
            trackEvents.forEach(function (trackEvent: any) {
    ...
    fetchSubtitlesUwp(track) {
        return Windows.Storage.StorageFile.getFileFromPathAsync(track.Path).then(function (storageFile: any) {
    ...
    renderSsaAss(videoElement, track, item) {
        ...
        import('@jellyfin/libass-wasm').then(({ default: SubtitlesOctopus }) => {
            ...
            const options = {
                ...
                onError() {
                    // WORKAROUND: JavascriptSubtitlesOctopus disposes itself on error but leaves
                    // a dangling reference. Clear it and defer error handling to allow cleanup.
    ...
    getCueCss(appearance, selector) {
        return `${selector}::cue {
                ${appearance.text.map((s: any) => s.value !== undefined && s.value !== '' ? `${s.name}:${s.value}!important;` : '').join('')}
            }`;
    }
    ...
    setCurrentTrackElement(streamIndex, targetTextTrackIndex) {
        ...
        let track = streamIndex === -1 ? null : mediaStreamTextTracks.filter(function (t: any) {
        ...
        sessionPromise.then((s: any) => {
            if (!s.TranscodingInfo || s.TranscodingInfo.IsVideoDirect) {
                ...
                mediaStreamTextTracks.forEach((t: any) => {
    ...
    getDeviceProfile(item, options) {
        return HtmlVideoPlayer.getDeviceProfileInternal(item, options).then((profile: any) => {
    ...
    setAirPlayEnabled(isEnabled) {
        ...
        if (document.AirPlayEnabled) {
            if (video) {
                if (isEnabled) {
                    video.requestAirPlay().catch(function (err: any) {
                        console.error('Error requesting AirPlay', err);
                    });
                } else {
                    document.exitAirPLay().catch(function (err: any) {
