...
function getDeviceHtml(device: TunerDevice): string {
    let html = '';
...
function renderDevices(view: HTMLElement, devices: TunerDevice[]): void {
    let html = '';
    ...
    const elem = view.querySelector('.results') as HTMLElement;
    elem.innerHTML = html;
...
class TunerPicker {
    show(): Promise<TunerDevice> {
        ...
        let deviceResult: TunerDevice | undefined;
        dlg.querySelector('.results')?.addEventListener('click', function (e: Event) {
            const target = (e as MouseEvent).target as HTMLElement;
            const tunerCard = dom.parentWithClass(target, 'card');
...
let currentDevices: TunerDevice[] = [];
