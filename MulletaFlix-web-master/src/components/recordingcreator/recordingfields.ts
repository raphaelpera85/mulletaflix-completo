...
interface RecordingFieldsOptions {
    parent: HTMLElement;
    programId: string;
    serverId: string;
}
...
function loadData(parent: Element, program: Program): void {
    if (program.IsSeries) {
...
function fetchData(instance: RecordingEditor): Promise<void> {
    ...
    return apiClient.getLiveTvProgram(options.programId, apiClient.getCurrentUserId()).then(function (program: Program) {
...
function onTimerChangedExternally(data: any, instance: RecordingEditor): void {
    const options = instance.options;
...
function onSeriesTimerChangedExternally(data: any, instance: RecordingEditor): void {
    const options = instance.options;
...
class RecordingEditor {
    options: RecordingFieldsOptions;
    TimerId?: string;
    Status?: string;
    SeriesTimerId?: string;
    changed?: boolean;
    private _unsubscribeTimers: (() => void)[];
    ...
    constructor(options: RecordingFieldsOptions) {
        ...
        this._unsubscribeTimers = [
            apiClient?.subscribe([OutboundWebSocketMessageType.TimerCreated], ({ Data }: { Data: any }) => onTimerChangedExternally(Data, this)),
            apiClient?.subscribe([OutboundWebSocketMessageType.TimerCancelled], ({ Data }: { Data: any }) => onTimerChangedExternally(Data, this)),
            apiClient?.subscribe([OutboundWebSocketMessageType.SeriesTimerCreated], ({ Data }: { Data: any }) => onSeriesTimerChangedExternally(Data, this)),
            apiClient?.subscribe([OutboundWebSocketMessageType.SeriesTimerCancelled], ({ Data }: { Data: any }) => onSeriesTimerChangedExternally(Data, this))
        ].filter(Boolean);
    }
...
function onManageRecordingClick(this: RecordingEditor): void {
...
function onRecordChange(this: RecordingEditor, e: Event): void {
    ...
    const button = dom.parentWithTag(e.target as HTMLElement, 'BUTTON');
    const isChecked = !(button as HTMLElement).querySelector('.material-icons')!.classList.contains('recordingIcon-active');
...
function onRecordSeriesChange(this: RecordingEditor, e: Event): void {
    this.changed = true;
