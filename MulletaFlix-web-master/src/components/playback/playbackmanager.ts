...
/** Minimal interface for player plugin instances used throughout PlaybackManager. */
interface MediaPlayer {
    ...
    seekRelative?(offsetTicks: number): void;
    seekable?(): boolean;

    getAudioStreamIndex?(): number;
    setAudioStreamIndex(index: number): void;
    getSubtitleStreamIndex?(): number;
    setSubtitleStreamIndex(index: number): void;
    getSecondarySubtitleStreamIndex?(): number;
    setSecondarySubtitleStreamIndex(index: number): void;
    changeAudioStream?(): void;
    changeSubtitleStream?(): void;
    canSetAudioStreamIndex?(): boolean;
    audioTracks?(): MediaStream[];
    subtitleTracks?(): MediaStream[];

    getDeviceProfile(item: ItemDto, options?: { isRetry?: boolean }): Promise<DeviceProfile>;
    supports(feature: string): boolean;

    getTargets?(): Promise<PlayTarget[]>;
    tryPair?(targetInfo: PlayTarget): Promise<void>;

    isFullscreen?(): boolean;
    toggleFullscreen?(): void;
    togglePictureInPicture?(): Promise<void>;
    toggleAirPlay?(): Promise<void>;
    ...
    setMaxStreamingBitrate?(options: unknown): void;
    ...
    getPlaylistSync?(): ItemDto[];
    getPlaylist?(): Promise<ItemDto[]>;
    setCurrentPlaylistItem?(playlistItemId: string): void;
    removeFromPlaylist?(playlistItemIds: string[]): Promise<void>;
    movePlaylistItem?(playlistItemId: string, newIndex: number): void;
    getCurrentPlaylistIndex?(): number;
    getCurrentPlaylistItemId?(): string;
    nextTrack?(): void;
    previousTrack?(): void;
    nextItem?(): Promise<ItemDto>;
    queueNext?(options: { items: ItemDto[] }): void;
    queue?(options: { items: ItemDto[] }): void;
    clearQueue?(clearCurrentItem?: boolean): void;
    setRepeatMode?(value: string): void;
    getRepeatMode?(): string;
    setQueueShuffleMode?(value: string): void;
    getQueueShuffleMode?(): string;

    getBufferedRanges?(): Array<[number, number]>;
    instantMix?(item: ItemDto): void;
    shuffle?(item: ItemDto): void;
    fastForward?(length?: number): void;
    rewind?(length?: number): void;
    playbackStartTime?(): number;
    displayContent?(options: unknown): void;
    beginPlayerUpdates?(): void;
    endPlayerUpdates?(): void;
    sendCommand?(cmd: { Name: string; Arguments: Record<string, unknown> }): void;
    enableMediaProbe?(item: ItemDto): boolean;
    supportsPlayMethod?(method: string, item: ItemDto): boolean;
    getDirectPlayProtocols?(): string[];
    playTrailers?(item: ItemDto): Promise<void>;
    canPlayMediaType(type: string): string;
    canPlayItem?(item: ItemDto, playOptions: unknown): boolean;
    canPlayUrl?(url: string): boolean;
    currentState?: Record<string, unknown>;
    _progressInterval?: ReturnType<typeof setInterval>;
}
...
function addPlaylistToPlaybackReport(playbackManagerInstance, info, player, serverId) {
    info.NowPlayingQueue = getPlaylistSync(playbackManagerInstance, player).map(function (i: any) {
        const itemInfo = {
            Id: i.Id,
...
function getIntros(firstItem, apiClient, options) {
    ...
    return apiClient.getIntros(firstItem.Id).then(function (result: any) {
        return result;
    }, function (error: any) {
...
function getAudioStreamUrlFromDeviceProfile(item, deviceProfile, maxBitrate, apiClient, startPosition) {
    const transcodingProfile = deviceProfile.TranscodingProfiles.filter(function (p: any) {
...
function getStreamUrls(items, deviceProfile, maxBitrate, apiClient, startPosition) {
    const audioTranscodingProfile = deviceProfile.TranscodingProfiles.filter(function (p: any) {
        return p.Type === 'Audio' && p.Context === 'Streaming';
    })[0];
...
function setStreamUrls(items, deviceProfile, maxBitrate, apiClient, startPosition) {
    return getStreamUrls(items, deviceProfile, maxBitrate, apiClient, startPosition).then(function (streamUrls: any) {
        for (let i = 0, length = items.length; i < length; i++) {
            const item = items[i];
...
function getOptimalMediaSource(apiClient, item, versions) {
    const promises = versions.map(function (v: any) {
    ...
    return Promise.all(promises).then(function (results: any) {
        for (let i = 0, length = versions.length; i < length; i++) {
            versions[i].enableDirectPlay = results[i] || false;
        }
        let optimalVersion = versions.filter(function (v: any) {
            return v.enableDirectPlay;
        })[0];

        if (!optimalVersion) {
            optimalVersion = versions.filter(function (v: any) {
                return v.SupportsDirectStream;
            })[0];
        }

        optimalVersion = optimalVersion || versions.filter(function (s: any) {
...
function isHostReachable(mediaSource, apiClient) {
    ...
    return apiClient.getEndpointInfo().then(function (endpointInfo: any) {
...
export class PlaybackManager {
    ...
    constructor() {
        ...
        self.currentItem = function (player: MediaPlayer) {
            if (!player) {
        ...
        self.currentMediaSource = function (player: MediaPlayer) {
            if (!player) {
                throw new Error('player cannot be null');
            }
        ...
        self.playMethod = function (player: MediaPlayer) {
            if (!player) {
                throw new Error('player cannot be null');
            }
        ...
        self.playSessionId = function (player: MediaPlayer) {
            if (!player) {
                throw new Error('player cannot be null');
            }
        ...
        self.setActivePlayer = function (player: string | MediaPlayer, targetInfo: PlayTarget) {
            ...
            if (typeof (player) === 'string') {
                player = players.filter(function (p: any) {
        ...
        self.trySetActivePlayer = function (player: string | MediaPlayer, targetInfo: PlayTarget) {
            if (player === 'localplayer' || player.name === 'localplayer') {
                if (self._currentPlayer?.isLocalPlayer) {
                    return;
                }
        ...
        self.getTargets = function () {
            ...
            return Promise.all(promises)
                .then((responses: any) => responses.flat().sort(sortPlayerTargets));
        };

        self.playerHasSecondarySubtitleSupport = function (player: MediaPlayer = self._currentPlayer) {
         ...
         */
        self.trackHasSecondarySubtitleSupport = function (track: MediaStream, player: MediaPlayer = self._currentPlayer) {
        ...
        self.secondarySubtitleTracks = function (player: MediaPlayer = self._currentPlayer) {
            const streams = self.subtitleTracks(player);
            return streams.filter((stream: MediaStream) => self.trackHasSecondarySubtitleSupport(stream, player));
        };
        ...
        self.getSubtitleStream = function (player: MediaPlayer, index: number) {
            return self.subtitleTracks(player).filter(function (s: MediaStream) {
                return s.Type === 'Subtitle' && s.Index === index;
            })[0];
        };

        self.getPlaylist = function (player: MediaPlayer) {
        ...
        self.promptToSkip = function (mediaSegment: unknown, player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.isPlaying = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.isPlayingMediaType = function (mediaType: string, player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.isPlayingLocally = function (mediaTypes: string[], player: MediaPlayer) {
            player = player || self._currentPlayer;
            ...
            return mediaTypes.filter(function (mediaType: string) {
                return self.isPlayingMediaType(mediaType, player);
            }).length > 0;
        };

        self.isPlayingVideo = function (player: MediaPlayer) {
            return self.isPlayingMediaType('Video', player);
        };

        self.isPlayingAudio = function (player: MediaPlayer) {
        ...
        self.canPlay = function (item: ItemDto) {
        ...
        self.toggleAspectRatio = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.setAspectRatio = function (val: string, player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.getSupportedAspectRatios = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.getAspectRatio = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.increasePlaybackRate = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
            if (player) {
        ...
        self.decreasePlaybackRate = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
            if (player) {
                const current = self.getPlaybackRate(player);
                const supported = self.getSupportedPlaybackRates(player);
        ...
        self.getSupportedPlaybackRates = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.setBrightness = function (val: number, player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.getBrightness = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.setVolume = function (val: number, player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.getVolume = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.volumeUp = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.volumeDown = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.changeAudioStream = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.changeSubtitleStream = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.getAudioStreamIndex = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.setAudioStreamIndex = function (index: number, player: MediaPlayer) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.getMaxStreamingBitrate = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.enableAutomaticBitrateDetection = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.setMaxStreamingBitrate = function (options: { maxBitrate?: number; enableAutomaticBitrateDetection?: boolean }, player: MediaPlayer) {
            player = player || self._currentPlayer;
            ...
            apiClient.getEndpointInfo().then(function (endpointInfo: any) {
                const playerData = getPlayerData(player);
                const mediaType = playerData.streamInfo ? playerData.streamInfo.mediaType : null;
        ...
        self.isFullscreen = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.toggleFullscreen = function (player: MediaPlayer) {
            player = player || self._currentPlayer;
        ...
        self.togglePictureInPicture = function (player: any) {
            player = player || self._currentPlayer;
            return player.togglePictureInPicture();
        };

        self.toggleAirPlay = function (player: any) {
            player = player || self._currentPlayer;
            return player.toggleAirPlay();
        };

        self.getSubtitleStreamIndex = function (player: any) {
            player = player || self._currentPlayer;
        ...
        self.getSecondarySubtitleStreamIndex = function (player: any) {
            player = player || self._currentPlayer;
        ...
        self.setSubtitleStreamIndex = function (index: any, player: any) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.setSecondarySubtitleStreamIndex = function (index: any, player: any) {
            player = player || self._currentPlayer;
        ...
        self.supportSubtitleOffset = function (player: any) {
            player = player || self._currentPlayer;
            return player && 'setSubtitleOffset' in player;
        };

        self.enableShowingSubtitleOffset = function (player: any) {
            player = player || self._currentPlayer;
            player.enableShowingSubtitleOffset();
        };

        self.disableShowingSubtitleOffset = function (player: any) {
            player = player || self._currentPlayer;
        ...
        self.isShowingSubtitleOffsetEnabled = function (player: any) {
            player = player || self._currentPlayer;
            return player.isShowingSubtitleOffsetEnabled();
        };

        self.isSubtitleStreamExternal = function (index: any, player: any) {
            const stream = self.getSubtitleStream(player, index);
            return stream ? getDeliveryMethod(stream) === 'External' : false;
        };

        self.setSubtitleOffset = function (value: any, player: any) {
            player = player || self._currentPlayer;
        ...
        self.getPlayerSubtitleOffset = function (player: any) {
            player = player || self._currentPlayer;
        ...
        self.seekRelative = function (offsetTicks: any, player: any) {
            player = player || self._currentPlayer;
        ...
        function changeStream(player, ticks, params) {
            ...
            }).then(function (deviceProfile: any) {
        ...
        function getNonItemTypePromise(firstItem, serverId, options, queryOptions) {
            if (firstItem.MediaType === 'Photo') {
                return getItemsForPlayback(serverId, mergePlaybackQueries({
                    ...
                    Limit: UNLIMITED_ITEMS
                }, queryOptions)).then(function (result: any) {
                    const playbackItems = result.Items;

                    let index = playbackItems.map(function (i: any) {
                        return i.Id;
        ...
        function getEpisodes(firstItem, options) {
            return new Promise(function (resolve: any, reject: any) {
                ...
                apiClient.getEpisodes(seriesId, {
                    ...
                    limit: 100,
                    startItemId: Id
                }).then(function (episodesResult: any) {
        ...
        self.getPlayerState = function (player: any, item: any, mediaSource: any) {
            player = player || self._currentPlayer;
        ...
        self.duration = function (player: any) {
            player = player || self._currentPlayer;
        ...
        const getAdditionalParts = async (items: any, mediaSourceId: any, startIndex: any) => {
            const getItemAndParts = async function (item: any, isStartItem: any) {
                if (
        ...
        function playWithIntros(items, options) {
            ...
            return getIntros(firstItem, apiClient, options).then(function (introsResult: any) {
                const introItems = (introsResult.Items || []).map(function (item: any) {
                    item.IsIntro = true;
                    return item;
                });
        ...
        function playAfterBitrateDetect(maxBitrate, item, playOptions, onPlaybackStartedFn, prevSource) {
            ...
            return Promise.all([promise, player.getDeviceProfile(item), apiClient.getCurrentUser(), getMediaStreams]).then(function (responses: any) {
                ...
                return getPlaybackMediaSource(player, apiClient, deviceProfile, item, mediaSourceId, options).then(async (mediaSource: any) => {
        ...
        self.getPlaybackInfo = function (item: any, options: any) {
            options = options || {};
            ...
            return apiClient.getEndpointInfo().then(function () {
                ...
                return player.getDeviceProfile(item).then(function (deviceProfile: any) {
                    const mediaOptions = {
                        maxBitrate,
                        startPosition,
                        isPlayback: null,
                    ...
                    return getPlaybackMediaSource(player, apiClient, deviceProfile, item, options.mediaSourceId, mediaOptions).then(function (mediaSource: any) {
        ...
        self.getPlaybackMediaSources = function (item: any, options: any) {
            options = options || {};
            const startPosition = options.startPositionTicks || 0;
            const mediaType = options.mediaType || item.MediaType;
            ...
            return apiClient.getEndpointInfo().then(function () {
                ...
                return player.getDeviceProfile(item).then(function (deviceProfile: any) {
                    ...
                    return getPlaybackInfo(player, apiClient, item, deviceProfile, null, null, mediaOptions).then(function (playbackInfoResult: any) {
        ...
        function getTextTracks(apiClient, item, mediaSource) {
            const subtitleStreams = mediaSource.MediaStreams.filter(function (s: any) {
                return s.Type === 'Subtitle';
            });

            const textStreams = subtitleStreams.filter(function (s: any) {
                    ...
                    return getOptimalMediaSource(apiClient, item, playbackInfoResult.MediaSources).then(function (mediaSource: any) {
                        if (mediaSource) {
                            if (mediaSource.RequiresOpening && !mediaSource.LiveStreamId) {
                                ...
                                return getLiveStream(player, apiClient, item, playbackInfoResult.PlaySessionId, deviceProfile, mediaSource, options).then(function (openLiveStreamResult: any) {
                                    return supportsDirectPlay(apiClient, item, openLiveStreamResult.MediaSource).then(function (result: any) {
                                ...
                                if (item.AlbumId != null) {
                                    return apiClient.getItem(apiClient.getCurrentUserId(), item.AlbumId).then(function (result: any) {
        ...
        function getPlayer(item, playOptions, forceLocalPlayers) {
            ...
            return getAutomaticPlayers(self, forceLocalPlayers).filter(function (p: any) {
        ...
        self.getItemFromPlaylistItemId = function (playlistItemId: any) {
            let item;
            let itemIndex;
        ...
        self.setCurrentPlaylistItem = function (playlistItemId: any, player: any) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.removeFromPlaylist = function (playlistItemIds: any, player: any) {
            if (!playlistItemIds) {
        ...
        self.movePlaylistItem = function (playlistItemId: any, newIndex: any, player: any) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.getCurrentPlaylistIndex = function (player: any) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.getCurrentPlaylistItemId = function (player: any) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.channelUp = function (player: any) {
            player = player || self._currentPlayer;
            return self.nextTrack(player);
        };

        self.channelDown = function (player: any) {
            player = player || self._currentPlayer;
        ...
        self.nextTrack = function (player: any) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.previousTrack = function (player: any) {
            player = player || self._currentPlayer;
            if (player && !enableLocalPlaylistManagement(player)) {
        ...
        self.queue = function (options: any, player: any = this._currentPlayer) {
            return queue(options, '', player);
        };

        self.queueNext = function (options: any, player: any = this._currentPlayer) {
        ...
        function initLegacyVolumeMethods(player) {
            ...
            player.setVolume = function (val: any) {
        ...
        function getLiveStreamMediaInfo(player, streamInfo, mediaSource, liveStreamId, serverId) {
            ...
            ServerConnections.getApiClient(serverId).getLiveStreamMediaInfo(liveStreamId).then(function (info: any) {
        ...
        self.playbackStartTime = function (player: any = this._currentPlayer) {
            if (player && !enableLocalPlaylistManagement(player) && !player.isLocalPlayer) {
    ...
    toggleMute(mute?: any, player = this._currentPlayer) {
        if (player) {
    ...
    enableDisplayMirroring(enabled?: any) {
        if (enabled != null) {
    ...
    nextChapter(player = this._currentPlayer) {
        ...
        const nextChapter = (item.Chapters || []).filter(function (i: any) {
    ...
    previousChapter(player = this._currentPlayer) {
        ...
        const previousChapters = (item.Chapters || []).filter(function (i: any) {
            return i.StartPositionTicks <= ticks;
        });
    ...
    seekFrames(frames = 1, player = this._currentPlayer) {
        ...
        const videoStream = source?.MediaStreams?.find((s: any) => s.Type === MediaType.Video);
    ...
    async playTrailers(item) {
        ...
        if (!items?.length) {
            items = (item.RemoteTrailers || []).map((t: any) => {
                return {
    ...
    instantMix(item, player = this._currentPlayer) {
        ...
        apiClient.getInstantMixFromItem(item.Id, options).then(function (result: any) {
            instance.play({
                items: result.Items
            });
        });
    ...
    audioTracks(player = this._currentPlayer) {
        ...
        const mediaStreams = mediaSource?.MediaStreams || [];
        return mediaStreams.filter(function (s: any) {
            return s.Type === 'Audio';
    ...
    trySetActiveDeviceName(name) {
        ...
        const instance = this;
        instance.getTargets().then(function (result: any) {
            const target = result.filter(function (p: any) {
