...
function getDeleteLabel(type: string): string {
    switch (type) {
...
export async function getCommands(options: ContextMenuOptions): Promise<ContextMenuCommand[]> {
    ...
    if (!browser.tv) {
        ...
        if (options.positionTo && (dom.parentWithClass(options.positionTo as HTMLElement, 'card') !== null)) {
            commands.push({
...
function getResolveFunction(resolve: (value: { command: string; updated?: boolean; deleted?: boolean; itemId?: string }) => void, commandId: string, changed?: boolean, deleted?: boolean, itemId?: string) {
    return function () {
        resolve({
            command: commandId,
            updated: changed,
        ...
        switch (id) {
            ...
            case 'multiSelect':
                import('./multiSelect/multiSelect').then(({ startMultiSelect }) => {
                    const card = dom.parentWithClass(options.positionTo as HTMLElement, 'card');
                    if (card) {
...
function play(item: ContextItem, resume?: boolean, queue?: boolean, queueNext?: boolean): void {
    let method = 'play';
    const playbackManagerAny = playbackManager as any;
    if (queue) {
        if (queueNext) {
            method = 'queueNext';
        } else {
...
function deleteItem(apiClient: any, item: ContextItem): Promise<any> {
    return new Promise<any>(function (resolve, reject) {
