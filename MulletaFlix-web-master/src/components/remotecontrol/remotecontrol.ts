...
interface PlaybackItem {
    ...
    Type?: string;
    MediaType?: string;
    MediaStreams?: Array<{ Type?: string }>;
    ServerId?: string;
    Name?: string;
    Album?: string | null;
    AlbumId?: string | null;
...
interface PlaybackState {
    NowPlayingItem?: PlaybackItem | null;
    PlayState?: {
        CanSeek?: boolean;
        IsPaused?: boolean;
        IsMuted?: boolean;
...
type RemoteControlConstructor = new () => RemoteControlModule;

interface SliderElement extends HTMLInputElement {
    dragging?: boolean;
    setKeyboardSteps(back: number, forward: number): void;
    setIsClear(clear: boolean): void;
    enableKeyboardDragging(): void;
    getBubbleText?: (value: number) => string;
}
...
function showAudioMenu(context: HTMLElement, player: PlaybackPlayer, button: HTMLElement): void {
...
function updateNowPlayingInfo(context: HTMLElement, state: PlaybackState, serverId?: string): void {
    ...
    if (item) {
        ...
        if (item.Type == 'AudioBook' || item.Type == 'Audio' || item.MediaStreams?.[0]?.Type == 'Audio') {
            ...
            requiredElement<HTMLElement>(context, '.nowPlayingSongName').innerText = item.Name || '';
        ...
        if (displayName.length > 0 && item.Type != 'Audio' && item.Type != 'Episode') {
            requiredElement<HTMLElement>(context, '.nowPlayingPageTitle').classList.remove('hide');
        } else {
            requiredElement<HTMLElement>(context, '.nowPlayingPageTitle').classList.add('hide');
        }
        ...
        let contextButton = requiredElement<HTMLButtonElement>(context, '.btnToggleContextMenu');
        ...
        contextButton = requiredElement<HTMLButtonElement>(context, '.btnToggleContextMenu');
...
function setImageUrl(context: HTMLElement, state: PlaybackState, url?: string | null): void {
    const item = state.NowPlayingItem;
...
function buttonVisible(btn: HTMLElement | null, enabled: boolean): void {
    if (!btn) {
        return;
    }
...
function updateSupportedCommands(context: HTMLElement, commands: string[]): void {
    const all = context.querySelectorAll<HTMLButtonElement>('.btnCommand');

    for (let i = 0, length = all.length; i < length; i++) {
        const command = all[i].getAttribute('data-command');
        const enableButton = command !== null && commands.indexOf(command) !== -1;
        all[i].disabled = !enableButton;
    }
}

const remotecontrol: RemoteControlConstructor = function (this: RemoteControlModule): void {
    ...
    function updatePlayerState(player: PlaybackPlayer, context: HTMLElement, state: PlaybackState): void {
        ...
        buttonVisible(requiredElement<HTMLElement>(context, '.btnToggleFullscreen'), item != null && item.MediaType == 'Video' && supportedCommands.includes('ToggleFullscreen'));
        ...
        if (supportedCommands.includes('DisplayMessage') && !currentPlayer.isLocalPlayer) {
            requiredElement<HTMLElement>(context, '.sendMessageSection').classList.remove('hide');
        } else {
            requiredElement<HTMLElement>(context, '.sendMessageSection').classList.add('hide');
        }

        if (supportedCommands.includes('SendString') && !currentPlayer.isLocalPlayer) {
            requiredElement<HTMLElement>(context, '.sendTextSection').classList.remove('hide');
        } else {
            requiredElement<HTMLElement>(context, '.sendTextSection').classList.add('hide');
        }

        if (supportedCommands.includes('Select') && !currentPlayer.isLocalPlayer) {
            requiredElement<HTMLElement>(context, '.navigationSection').classList.remove('hide');
        } else {
            requiredElement<HTMLElement>(context, '.navigationSection').classList.add('hide');
        }

        if (isSupportedCommands && !currentPlayer.isLocalPlayer) {
            requiredElement<HTMLElement>(context, '.remoteControlSection').classList.remove('hide');
        } else {
            requiredElement<HTMLElement>(context, '.remoteControlSection').classList.add('hide');
        }

        buttonVisible(requiredElement<HTMLElement>(context, '.btnLyrics'), item?.Type === 'Audio' && !layoutManager.mobile);
        buttonVisible(requiredElement<HTMLElement>(context, '.btnStop'), item != null);
        buttonVisible(requiredElement<HTMLElement>(context, '.btnNextTrack'), item != null);
        buttonVisible(requiredElement<HTMLElement>(context, '.btnPreviousTrack'), item != null);
        if (layoutManager.mobile) {
            ...
            buttonVisible(requiredElement<HTMLElement>(context, '.btnRepeat'), playingAudio && !playingAudioBook);
            buttonVisible(requiredElement<HTMLElement>(context, '.btnShuffleQueue'), playingAudio && !playingAudioBook);
            buttonVisible(requiredElement<HTMLElement>(context, '.btnRewind'), playingVideo || playingAudioBook);
            buttonVisible(requiredElement<HTMLElement>(context, '.btnFastForward'), playingVideo || playingAudioBook);
            buttonVisible(requiredElement<HTMLElement>(context, '.nowPlayingSecondaryButtons .btnShuffleQueue'), playingVideo);
            buttonVisible(requiredElement<HTMLElement>(context, '.nowPlayingSecondaryButtons .btnRepeat'), playingVideo);
        } else {
            buttonVisible(requiredElement<HTMLElement>(context, '.btnRewind'), item != null);
            buttonVisible(requiredElement<HTMLElement>(context, '.btnFastForward'), item != null);
        }
        const positionSlider = requiredElement<SliderElement>(context, '.nowPlayingPositionSlider');
    ...
    function updateAudioTracksDisplay(player: PlaybackPlayer, context: HTMLElement): void {
        const supportedCommands = currentPlayerSupportedCommands;
        buttonVisible(requiredElement<HTMLElement>(context, '.btnAudioTracks'), playbackManager.audioTracks(player).length > 1 && supportedCommands.indexOf('SetAudioStreamIndex') != -1);
    }

    function updateSubtitleTracksDisplay(player: PlaybackPlayer, context: HTMLElement): void {
        const supportedCommands = currentPlayerSupportedCommands;
        buttonVisible(requiredElement<HTMLElement>(context, '.btnSubtitles'), playbackManager.subtitleTracks(player).length && supportedCommands.indexOf('SetSubtitleStreamIndex') != -1);
    }

    function updateRepeatModeDisplay(repeatMode: 'RepeatAll' | 'RepeatOne' | 'RepeatNone'): void {
        const context = dlg;
        const toggleRepeatButtons = context.querySelectorAll<HTMLElement>('.repeatToggleButton');
    ...
    function updatePlayerVolumeState(context: HTMLElement, isMuted: boolean, volumeLevel: number | undefined): void {
        const view = context;
        const supportedCommands = currentPlayerSupportedCommands;
        ...
        const buttonMute = requiredElement<HTMLButtonElement>(view, '.buttonMute');
        const buttonMuteIcon = requiredElement<HTMLElement>(buttonMute, '.material-icons');
        ...
        if (!showMuteButton && !showVolumeSlider) {
            requiredElement<HTMLElement>(context, '.volumecontrol').classList.add('hide');
        } else {
            buttonMute.classList.toggle('hide', !showMuteButton);

            const nowPlayingVolumeSlider = requiredElement<SliderElement>(context, '.nowPlayingVolumeSlider');
            const nowPlayingVolumeSliderContainer = requiredElement<HTMLElement>(context, '.nowPlayingVolumeSliderContainer');
    ...
    function updatePlayPauseState(isPaused: boolean, isActive: boolean): void {
        const context = dlg;
        const btnPlayPause = requiredElement<HTMLButtonElement>(context, '.btnPlayPause');
        const btnPlayPauseIcon = requiredElement<HTMLElement>(btnPlayPause, '.material-icons');
        ...
        const playlistIndicator = context.querySelector<HTMLElement>('.playlistIndexIndicatorImage');
    ...
    function updateTimeDisplay(positionTicks: number | undefined, runtimeTicks: number | null): void {
        const context = dlg;
        const positionSlider = requiredElement<SliderElement>(context, '.nowPlayingPositionSlider');
        ...
        requiredElement<HTMLElement>(context, '.positionTime').innerHTML = Number.isFinite(positionTicks) ? datetime.getDisplayRunningTime(positionTicks) : '--:--';
        requiredElement<HTMLElement>(context, '.runtime').innerHTML = Number.isFinite(runtimeTicks) ? datetime.getDisplayRunningTime(runtimeTicks) : '--:--';
    }

    function getPlaylistItems(player: PlaybackPlayer): Promise<Array<{ Id: string; ServerId?: string }>> {
    ...
    function loadPlaylist(context: HTMLElement, player: PlaybackPlayer): void {
        getPlaylistItems(player).then(function (items) {
            ...
            if (layoutManager.mobile) {
                if (items.length > 0) {
                    requiredElement<HTMLElement>(context, '.btnTogglePlaylist').classList.remove('hide');
                } else {
                    requiredElement<HTMLElement>(context, '.btnTogglePlaylist').classList.add('hide');
                }
            ...
            const itemsContainer = requiredElement<HTMLElement>(context, '.playlist');
            const focusedItemPlaylist = itemsContainer.querySelector<HTMLButtonElement>('button:focus');
            itemsContainer.innerHTML = html;
            if (focusedItemPlaylist) {
                const focusedItemPlaylistId = focusedItemPlaylist.getAttribute('data-playlistitemid');
