...
interface ChapterMediaStream {
    Type?: string;
    Width?: number;
    Height?: number;
}

interface ChapterCardItem {
    Id: string;
    IsFolder?: boolean;
    MediaSources?: Array<{
        MediaStreams?: ChapterMediaStream[];
    }>;
    MediaType?: string;
    ServerId: string;
    Type?: string;
...
interface ChapterCardBuilderOptions {
...
type ChapterCardApiClient = NonNullable<ReturnType<typeof ServerConnections.getApiClient>> & {
    getScaledImageUrl(itemId: string, options: {
        index: number;
        maxWidth: number;
        tag?: string;
        type: string;
...
function buildChapterCardsHtml(item: ChapterCardItem, chapters: ChapterCard[], options: ChapterCardBuilderOptions): string {
    ...
    const videoStream = mediaStreams.find(({ Type }) => Type === 'Video');
    ...
    const apiClient = ServerConnections.getApiClient(item.ServerId) as ChapterCardApiClient;
...
function getImgUrl(item: ChapterCardItem, chapter: ChapterCard, index: number, maxWidth: number, apiClient: ChapterCardApiClient): string | null {
    if (chapter.ImageTag) {
...
function buildChapterCard(
    ...
    apiClient: ChapterCardApiClient,
    chapter: ChapterCard,
    index: number,
    options: Pick<ChapterCardBuilderOptions, 'width' | 'coverImage'>,
    className: string,
    shape: string
): string {
    const imgUrl = getImgUrl(item, chapter, index, options.width || 400, apiClient);
...
export function buildChapterCards(item: ChapterCardItem, chapters: ChapterCard[], options: ChapterCardBuilderOptions): void | undefined {
