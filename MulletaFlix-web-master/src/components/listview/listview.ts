...
interface ListViewOptions extends Omit<ListOptions, 'items' | 'action' | 'context' | 'sortBy'> {
    items?: ItemDto[] | null;
    ...
    artist?: boolean;
    rightButtons?: ListViewButton[];
}

function getIndex(item: ItemDto, options: Pick<ListViewOptions, 'index' | 'sortBy'>): string {
    if (options.index === 'disc') {
        return item.ParentIndexNumber == null ? '' : globalize.translate('ValueDiscNumber', String(item.ParentIndexNumber));
    }
...
function getImageUrl(item: ItemDto, size: number): string | null {
    type ListViewApiClient = {
        getScaledImageUrl: (
            itemId: string | { ServerId?: string; Id?: string },
            options: {
                fillWidth: number;
                fillHeight: number;
                type: 'Primary';
...
function getChannelImageUrl(item: ItemDto, size: number): string | null {
    type ListViewApiClient = {
        getScaledImageUrl: (
            itemId: string | { ServerId?: string; Id?: string },
            options: {
...
function getTextLinesHtml(textlines: string[], isLargeStyle: boolean): string {
    let html = '';
...
function getRightButtonsHtml(rightButtons: ListViewButton[]): string {
    let html = '';
...
export function getListViewHtml(options: ListViewOptions): string {
    ...
    for (let i = 0, length = items.length; i < length; i++) {
        ...
        if (options.showProgramDateTime && startDate) {
            textlines.push(datetime.toLocaleString(datetime.parseISO8601Date(startDate), {
        ...
        if (options.showProgramTime && startDate) {
            textlines.push(datetime.getDisplayTime(datetime.parseISO8601Date(startDate)) ?? '');
        }
        ...
        if (!clickEntireItem) {
            ...
            if (options.rightButtons) {
                html += getRightButtonsHtml(options.rightButtons);
            }
