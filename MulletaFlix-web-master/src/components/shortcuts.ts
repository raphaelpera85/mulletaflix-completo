...
function playAllFromHere(card: any, serverId: string, queue?: boolean): any {
    ...
    const itemsContainer: any = dom.parentWithClass(card, 'itemsContainer');
    if (itemsContainer?.fetchData) {
        const queryOptions = queue ? { StartIndex: startIndex } : {};

        return itemsContainer.fetchData(queryOptions).then((result: any) => {
            if (queue) {
...
function notifyRefreshNeeded(childElement: any, itemsContainer: any): void {
...
function showContextMenu(card: any, options: any = {}): void {
    getItem(card).then((item: any) => {
        ...
        if (playlistId) {
            ...
            (item as any).PlaylistItemId = elem ? elem.getAttribute('data-playlistitemid') : null;
        ...
        Promise.all([
            ...
            .then(([
                ...
                return itemContextMenu.show(menuOptions);
            })
            .then((result: any) => {
...
function showPlayMenu(card: any, target: any): void {
...
function executeAction(card: any, target: any, action: string): void {
    ...
    switch (action) {
        ...
        case ItemAction.Menu: {
            const options: any = target.getAttribute('data-playoptions') === 'false' ?
                {
                    shuffle: false,
                    instantMix: false,
                    play: false,
...
function addToPlaylist(item: any): void {
...
export function onClick(e: any): boolean | undefined {
    const card: any = dom.parentWithClass(e.target, 'itemAction');

    if (card) {
        let actionElement: any = card;
...
export function on(context: any, options?: any): void {
    options = options || {};
...
export function off(context: any, options?: any): void {
    options = options || {};
