...
const copyButtonHtml: string = layoutManager.tv ? '' :
...
const attributeDelimiterHtml: string = layoutManager.tv ? '' : '<span class="hide">: </span>';

function setMediaInfo(user: any, page: HTMLElement, item: any): void {
    let html = item.MediaSources.map((version: any) => {
        return getMediaSourceHtml(user, item, version);
    }).join('<div style="border-top:1px solid #444;margin: 1em 0;"></div>');
    if (item.MediaSources.length > 1) {
        html = `<br/>${html}`;
    }
    const mediaInfoContent = page.querySelector('#mediaInfoContent') as HTMLElement;
    mediaInfoContent.innerHTML = html;

    for (const btn of mediaInfoContent.querySelectorAll('.btnCopy')) {
        btn.addEventListener('click', () => {
            const infoBlock = dom.parentWithClass(btn as HTMLElement, 'mediaInfoStream') || dom.parentWithClass(btn as HTMLElement, 'mediaInfoSource') || mediaInfoContent;

            copy((infoBlock as HTMLElement).textContent || '').then(() => {
...
function getMediaSourceHtml(user: any, _item: any, version: any): string {
    ...
    for (const stream of version.MediaStreams) {
        ...
        let translateString: string | undefined;
        switch (stream.Type) {
            case 'Audio':
            case 'Data':
            case 'Subtitle':
        ...
        if (stream.BitRate) {
            attributes.push(createAttribute(globalize.translate('MediaInfoBitrate'), `${parseInt(String(stream.BitRate / 1000), 10)} kbps`));
        }
...
function createAttribute(label: string, value: any, isLtr?: boolean): string {
