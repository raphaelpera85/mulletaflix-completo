...
interface MediaInfoItem {
    ...
    ProgramInfo?: MediaInfoItem | null;
...
interface MediaInfoOptions {
    ...
    year?: boolean;
    programIndicator?: boolean;
    episodeTitle?: boolean;
    episodeTitleIndexNumber?: boolean;
    runtime?: boolean;
...
interface MediaInfoTextItem {
    text: string | number;
    cssClass?: string;
}

interface MediaInfoHtmlItem {
    html: string;
    cssClass?: string;
}

type MediaInfoValue = string | number | MediaInfoTextItem | MediaInfoHtmlItem;

interface MediaInfoStat {
    type: 'mediainfo' | 'added';
    text: string | number;
}

function getTimerIndicator(item: MediaInfoItem): string {
...
function getProgramInfoHtml(item: MediaInfoItem, options: MediaInfoOptions): string {
    let html = '';

    const miscInfo: MediaInfoValue[] = [];
...
export function getMediaInfoHtml(item: MediaInfoItem, options: MediaInfoOptions = {}): string {
    let html = '';
    ...
    if (showFolderRuntime) {
        ...
        if (count) {
            miscInfo.push(globalize.translate('TrackCount', count.toString()));
        }
        ...
        if (count) {
            miscInfo.push(globalize.translate('ItemCount', count.toString()));
        }
    }
    ...
    if (options.year !== false && item.ProductionYear && item.Type === 'Series') {
        if (item.Status === 'Continuing') {
            miscInfo.push(globalize.translate('SeriesYearToPresent', item.ProductionYear.toString()));
        } else if (item.ProductionYear) {
            text = item.ProductionYear.toString();

            if (item.EndDate) {
                try {
                    const endYear = datetime.parseISO8601Date(item.EndDate).getFullYear().toString();
    ...
    if (item.Type === 'Program' || item.Type === 'Timer') {
        let program: MediaInfoItem = item;
        ...
        if ((program.IsSeries || program.EpisodeTitle) && options.episodeTitle !== false) {
            ...
            if (text) {
                miscInfo.push(escapeHtml(text.toString()));
            }
            ...
            miscInfo.push(program.ProductionYear.toString());
        }
    }
    ...
    if (options.year !== false && item.Type !== 'Series' && item.Type !== 'Episode' && item.Type !== 'Person'
        ...
        if (item.ProductionYear) {
            ...
            try {
                text = datetime.parseISO8601Date(item.PremiereDate).getFullYear().toString();
                miscInfo.push(text);
            } catch (e: unknown) {
                console.error('error parsing date:', item.PremiereDate, e);
            }
...
export function getEndsAt(item: MediaInfoItem): string | null {
...
export function getEndsAtFromPosition(runtimeTicks: number, positionTicks: number | null | undefined, playbackRate: number, includeText?: boolean): string {
    const endDate = new Date(new Date().getTime() + (1 / playbackRate) * ((runtimeTicks - (positionTicks || 0)) / 10000));
...
function getMediaInfoItem(m: MediaInfoValue, cssClass?: string): string {
    cssClass = cssClass ? (`${cssClass} mediaInfoItem`) : 'mediaInfoItem';
    let mediaInfoText: string | number | undefined;
...
function dynamicEndTime(elem: HTMLElement, item: MediaInfoItem): void {
...
export function fillPrimaryMediaInfo(elem: HTMLElement, item: MediaInfoItem, options: MediaInfoOptions = {}): void {
...
export function fillSecondaryMediaInfo(elem: HTMLElement, item: MediaInfoItem, options: MediaInfoOptions = {}): void {
...
function afterFill(elem: HTMLElement, item: MediaInfoItem, options: MediaInfoOptions): void {
    if (options.endsAt !== false) {
        const endsAtElem = elem.querySelector<HTMLElement>('.endsAt');
        if (endsAtElem) {
            dynamicEndTime(endsAtElem, item);
        }
    }

    const lnkChannel = elem.querySelector<HTMLElement>('.lnkChannel');
    if (lnkChannel) {
        lnkChannel.addEventListener('click', onChannelLinkClick);
    }
}

function onChannelLinkClick(this: HTMLElement, e: MouseEvent): boolean {
...
export function getPrimaryMediaInfoHtml(item: MediaInfoItem, options: MediaInfoOptions = {}): string {
...
export function getSecondaryMediaInfoHtml(item: MediaInfoItem, options: MediaInfoOptions = {}): string {
...
export function getResolutionText(i: MediaStreamInfo): string | null {
    const width = i.Width;
...
function getAudioStreamForDisplay(item: MediaInfoItem): MediaStreamInfo | null {
...
export function getMediaInfoStats(item: MediaInfoItem): MediaInfoStat[] {
    const list: MediaInfoStat[] = [];

    const mediaSource: MediaSourceInfo = (item.MediaSources || [])[0] || {};

    const videoStream: MediaStreamInfo = (mediaSource.MediaStreams || []).filter(i => {
        return i.Type === 'Video';
    })[0] || {};
