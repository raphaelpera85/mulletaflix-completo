...
export function getFetchPromise(request: FetchRequest): Promise<Response> {
    ...
    const fetchRequest: RequestInit & { headers: Record<string, string> } = {
        headers: headers,
        method: request.type,
...
/**
     * @param params - Record of key-value pairs.
...
export function ajax(request: FetchRequest): Promise<unknown> {
    if (!request) {
    ...
    return getFetchPromise(request).then(function (response: Response) {
