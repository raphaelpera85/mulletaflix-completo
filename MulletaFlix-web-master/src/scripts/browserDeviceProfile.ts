...
declare const webapis: { productinfo: { isUdPanelSupported: () => boolean } };
...
interface TranscodingProfile {
    ...
    ApplyConditions?: CodecCondition[];
}

interface CodecCondition {
    Condition: string;
    Property: string;
    Value: string;
    IsRequired?: boolean;
...
function canPlayHevc(videoTestElement: HTMLVideoElement, options: ProfileOptions): boolean | string {
...
function canPlayAv1(videoTestElement: HTMLVideoElement): boolean | string {
 ...
 * @param videoTestElement The video test element
 * @returns _true_ if the device supports DTS (DCA). _false_ if the device doesn't support DTS (DCA). _null_ if support status is unknown.
 */
...
function supportsEac3(videoTestElement: HTMLVideoElement): boolean | string {
...
function supportsMp3InHls(videoTestElement: HTMLVideoElement): boolean | string {
    if (videoTestElement.canPlayType) {
...
function canPlayAudioFormat(format: string): boolean | string {
...
function supportsVc1(videoTestElement: HTMLVideoElement): boolean | string {
    return browser.tizen || browser.web0s || browser.edgeUwp || videoTestElement.canPlayType('video/mp4; codecs="vc-1"').replace(/no/, '');
}

function supportsHdr10(options: ProfileOptions): boolean {
...
function supportsHlg(options: ProfileOptions): boolean {
    return options.supportsHlg ?? supportsHdr10(options);
}

function supportsDolbyVision(options: ProfileOptions): boolean {
    // eslint-disable-next-line no-constant-binary-expression, sonarjs/no-redundant-boolean
...
function supportedDolbyVisionProfilesHevc(videoTestElement: HTMLVideoElement): number[] {
...
function supportedDolbyVisionProfileAv1(videoTestElement: HTMLVideoElement): string {
    // Profile 10 4k@24fps
    return videoTestElement.canPlayType?.('video/mp4; codecs="dav1.10.06"').replace(/no/, '') as string;
}

function supportsAnamorphicVideo(): boolean {
...
function getGlobalMaxVideoBitrate(): number | null {
    ...
    if (browser.tizen) {
        try {
            ...
            console.error('isUdPanelSupported() error code = ' + (error as Record<string, unknown>).code);
        }
    }
...
function getSpeakerCount(): number {
...
function getPhysicalAudioChannels(options: ProfileOptions, videoTestElement: HTMLVideoElement): number {
 ...
 * @param videoTestElement The video test element
 * @returns _true_ if the web engine supports secondary audio.
 */
export function canPlaySecondaryAudio(videoTestElement: HTMLVideoElementWithAudioTracks): boolean {
...
export default function (options?: ProfileOptions): DeviceProfile {
    options = options || {};
    ...
    let maxVideoWidth: number | null = browser.xboxOne ? window.screen?.width ?? null : null;
    ...
    let supportsDts: boolean | null | undefined = appSettings.enableDts() || options.supportsDts;
    if (supportsDts == null) {
    ...
    videoAudioCodecs = videoAudioCodecs.filter(function (c: string) {
        return (options.disableVideoAudioCodecs || []).indexOf(c) === -1;
    });

    hlsInTsVideoAudioCodecs = hlsInTsVideoAudioCodecs.filter(function (c: string) {
        return (options.disableHlsVideoAudioCodecs || []).indexOf(c) === -1;
    });

    hlsInFmp4VideoAudioCodecs = hlsInFmp4VideoAudioCodecs.filter(function (c: string) {
        return (options.disableHlsVideoAudioCodecs || []).indexOf(c) === -1;
    });
    ...
    ['m2ts', 'wmv', 'ts', 'asf', 'avi', 'mpg', 'mpeg', 'flv', '3gp', 'mts', 'trp', 'vob', 'vro', 'mov'].map(function (container: string) {
        return getDirectPlayProfileForVideoContainer(container, videoAudioCodecs, videoTestElement, options);
    }).filter(function (i: DirectPlayProfile | null) {
        return i != null;
    }).forEach(function (i) {
        profile.DirectPlayProfiles.push(i!);
    });

    ['opus', 'mp3', 'mp2', 'aac', 'flac', 'alac', 'webma', 'wma', 'wav', 'ogg', 'oga'].filter(canPlayAudioFormat as (value: string) => boolean).forEach(function (audioFormat: string) {
    ...
    ['aac', 'mp3', 'opus', 'wav'].filter(canPlayAudioFormat as (value: string) => boolean).forEach(function (audioFormat: string) {
        profile.TranscodingProfiles.push({
            Container: audioFormat,
            Type: 'Audio',
            AudioCodec: audioFormat,
    ...
    ['opus', 'mp3', 'aac', 'wav'].filter(canPlayAudioFormat as (value: string) => boolean).forEach(function (audioFormat: string) {
        profile.TranscodingProfiles.push({
            Container: audioFormat,
            Type: 'Audio',
            AudioCodec: audioFormat,
    ...
    if (canPlayHls() && options.enableHls !== false) {
        ...
        if (hlsInFmp4VideoCodecs.length && hlsInFmp4VideoAudioCodecs.length && enableFmp4Hls) {
            // WORKAROUND: API lacks TS/MP4 container filter for HLS. Declare general HLS support
            // and rely on DirectPlay retry fallback. See TODO for proper Container+Protocol filter.
            // TODO: Implement {Container: 'mp4', Protocol: 'hls'} or {Container: 'hls', SubContainer: 'mp4'}
            profile.DirectPlayProfiles.push({
                Container: 'hls',
                Type: 'Video',
        ...
        if (hlsInTsVideoCodecs.length && hlsInTsVideoAudioCodecs.length) {
            // WORKAROUND: Same as above — TS variant for HLS without container-specific API filter.
            // TODO: Implement {Container: 'ts', Protocol: 'hls'} or {Container: 'hls', SubContainer: 'ts'}
            profile.DirectPlayProfiles.push({
                Container: 'hls',
                Type: 'Video',
    ...
    const aacCodecProfileConditions: CodecCondition[] = [];
    ...
    const globalAudioCodecProfileConditions: CodecCondition[] = [];
    const globalVideoAudioCodecProfileConditions: CodecCondition[] = [];
    ...
    if (browser.web0s) {
        const flacConditions: CodecCondition[] = [
        ...
        const flacTranscodingProfiles: TranscodingProfile[] = [];

        // Split each video transcoding profile with FLAC so that the containing FLAC is only applied to 2 channels audio
        profile.TranscodingProfiles.forEach((transcodingProfile: TranscodingProfile) => {
            ...
            const flacTranscodingProfile: TranscodingProfile = { ...transcodingProfile };
            ...
            transcodingProfile.AudioCodec = audioCodecs.filter((codec: string) => codec != 'flac').join(',');
        });
    ...
    if (safariSupportsOpus) {
        const opusConditions: CodecCondition[] = [
        ...
        const opusTranscodingProfiles: TranscodingProfile[] = [];
        ...
        profile.TranscodingProfiles.forEach((transcodingProfile: TranscodingProfile) => {
            ...
            const opusTranscodingProfile: TranscodingProfile = { ...transcodingProfile };
            ...
            transcodingProfile.AudioCodec = audioCodecs.filter((codec: string) => codec != 'opus').join(',');
        });
    ...
    if ((browser.tizenVersion >= 5 && (window as unknown as Record<string, unknown>).NativeShell)
    ...
    const h264CodecProfileConditions: CodecCondition[] = [
        {
            Condition: 'EqualsAny',
    ...
    const hevcCodecProfileConditions: CodecCondition[] = [
        {
            Condition: 'EqualsAny',
            Property: 'VideoProfile',
            Value: hevcProfiles,
    ...
    const vp9CodecProfileConditions: CodecCondition[] = [
        {
            Condition: 'EqualsAny',
            Property: 'VideoRangeType',
    ...
    const av1CodecProfileConditions: CodecCondition[] = [
        {
            Condition: 'EqualsAny',
            Property: 'VideoProfile',
            Value: av1Profiles,
    ...
    if (browser.iOS && browser.iOSVersion < 13) {
        const codecProfileTS: CodecProfile = {
            Type: 'Video',
            Codec: 'h264',
            Container: 'ts',
            Conditions: h264CodecProfileConditions.filter((condition: CodecCondition) => {
        ...
        const codecProfileMp4: CodecProfile = {
            Type: 'Video',
            Codec: 'h264',
            Container: 'mp4',
            Conditions: h264CodecProfileConditions.filter((condition: CodecCondition) => {
            ...
            Conditions: [
                {
                    Condition: 'EqualsAny',
                    Property: 'VideoRangeType',
                    Value: hevcVideoRangeTypes.split('|').filter((v: string) => !v.startsWith('DOVI')).join('|'),
                    IsRequired: false
                }
            ]
        });
    ...
    const globalVideoConditions: CodecCondition[] = [];
