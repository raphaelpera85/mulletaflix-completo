...
export function parseISO8601Date(s: string, toLocal?: boolean): Date {
    // parenthese matches:
    ...
    if ((d[7] as number) > 0) {
        ms += Math.round((d[7] as number) * 1000);
    }
...
const toLocaleTimeStringSupportsLocales = function (): boolean {
    try {
...
function getOptionList(options: Record<string, string>): Array<{ name: string; value: string }> {
...
export function toLocaleDateString(date: Date, options?: Intl.DateTimeFormatOptions): string {
    ...
    const optionList = getOptionList(options as Record<string, string>);
...
export function getDisplayDateTime(date: Date | string): string {
    ...
    if (typeof date === 'string') {
        try {
            return toLocaleString(parseISO8601Date(date, true));
        } catch {
            return date;
        }
    }
...
export function getDisplayTime(date: Date | string): string {
    ...
    if (typeof date === 'string') {
        try {
            dateObj = parseISO8601Date(date, true);
        } catch {
            return date;
        }
    } else {
    ...
    if (toLocaleTimeStringSupportsLocales) {
        return toLocaleTimeString(dateObj, {
