...
function renderItems(page: HTMLElement, item: ItemData): void {
    ...
    for (let i = 0, length = sectionElems.length; i < length; i++) {
        renderSection(item, sectionElems[i] as HTMLElement, sectionElems[i].getAttribute('data-type'));
    }
}

function renderSection(item: ItemData, element: HTMLElement, type: string | null): void {
    switch (type) {
        case 'Program':
...
function getMoreItemsHref(item: ItemData, type: string | null): string {
    if (item.Type === 'Genre') {
...
function addCurrentItemToQuery(query: QueryOptions, item: ItemData): void {
    switch (item.Type) {
...
function getQuery(options: QueryOptions, item: ItemData): QueryOptions {
