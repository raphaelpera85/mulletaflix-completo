...
// The browser object is a dynamic bag of properties set at runtime based on UA detection.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type BrowserObject = Record<string, any>;
...
function hasKeyboard(browser: BrowserObject): boolean {
    if (browser.touch) {
        return true;
    }
...
function web0sVersion(browser: BrowserObject): number | undefined {
...
function supportsCssAnimation(allowPrefix?: boolean): boolean {
    ...
    if (animation === false && allowPrefix) {
        for (const domPrefix of domPrefixes) {
            if ((elm.style as unknown as Record<string, unknown>)[domPrefix + 'AnimationName'] !== undefined) {
                animation = true;
                break;
            }
        }
...
const uaMatch = function (ua: string): UaMatch {
    ...
    let platformMatch: RegExpExecArray | string[] = /(ipad)/.exec(ua)
        || /(iphone)/.exec(ua)
        || /(windows)/.exec(ua)
        || /(android)/.exec(ua)
        || /(titanos)/.exec(ua)
...
export const detectBrowser = (userAgent: string = navigator.userAgent): BrowserObject => {
    ...
    browser.tizen = normalizedUA.includes('tizen') || (window as unknown as Record<string, unknown>).tizen != null;
