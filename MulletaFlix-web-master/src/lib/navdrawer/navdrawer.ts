...
interface NavDrawerOptions {
    target: HTMLElement;
    width: number;
    handleSize: number;
    disableMask: boolean;
    maxMaskOpacity: number;
...
function getTouches(e: Event): TouchList {
    const te = e as TouchEvent;
    return te.changedTouches || te.targetTouches || te.touches;
}

function disableEvent(e: Event): void {
    e.preventDefault();
    e.stopPropagation();
}
...
class NavDrawer {
    ...
    velocity = 0;
    dragMode = 0;
    menuTouchStartX!: number;
    menuTouchStartY!: number;
    menuTouchStartTime!: number;
    edgeContainer = document.querySelector('.mainDrawerHandle') as HTMLElement;
    isPeeking = false;
    backgroundTouchStartX!: number;
    backgroundTouchStartTime!: number;
    _edgeSwipeEnabled?: boolean;
    options!: NavDrawerOptions;
    defaults!: NavDrawerOptions;
    scrollContainer!: HTMLElement;
    isVisible = false;

    constructor(options: NavDrawerOptions) {
        ...
        this.scrollContainer = options.target.querySelector('.mainDrawer-scrollContainer') as HTMLElement;
        this.scrollContainer.classList.add('scrollY');
        this.isVisible = false;
        this.initialize();
    }

    setVelocity(deltaX: number): void {
        const time = new Date().getTime() - (this.menuTouchStartTime || 0);
        this.velocity = Math.abs(deltaX) / time;
    }

    onMenuTouchStart = (e: Event): void => {
        const options = this.options;

        options.target.classList.remove('transition');
        const touches = getTouches(e);
        const touch: Partial<Touch> = touches[0] || {};
        this.menuTouchStartX = touch.clientX || 0;
        this.menuTouchStartY = touch.clientY || 0;
        this.menuTouchStartTime = new Date().getTime();
    };

    onMenuTouchMove = (e: Event): void => {
        ...
        if (this.dragMode === 0 && (!isOpen || Math.abs(deltaX) >= 10) && Math.abs(deltaY) < 5) {
            this.dragMode = 1;
            scrollContainer.addEventListener('scroll', disableEvent as EventListener);
            this.showMask();
    ...
    onMenuTouchEnd = (e: Event): void => {
        const options = this.options;
        const scrollContainer = this.scrollContainer;

        options.target.classList.add('transition');
        scrollContainer.removeEventListener('scroll', disableEvent as EventListener);
        this.dragMode = 0;
        const touches = getTouches(e);
        const touch: Partial<Touch> = touches[0] || {};
        const endX = touch.clientX || 0;
    ...
    onEdgeTouchMove = (e: Event): void => {
        e.preventDefault();
        e.stopPropagation();
        this.onEdgeTouchStart(e);
    };

    onEdgeTouchStart = (e: Event): void => {
        const options = this.options;
    ...
    onBackgroundTouchStart = (e: Event): void => {
        const touches = getTouches(e);
        const touch: Partial<Touch> = touches[0] || {};
        this.backgroundTouchStartX = touch.clientX || 0;
        this.backgroundTouchStartTime = new Date().getTime();
    };

    onBackgroundTouchMove = (e: Event): void => {
        const options = this.options;
    ...
    onBackgroundTouchEnd = (e: Event): void => {
        const touches = getTouches(e);
        const touch: Partial<Touch> = touches[0] || {};
        const endX = touch.clientX || 0;
        let deltaX = endX - (this.backgroundTouchStartX || 0);
    ...
    animateToPosition(pos: number): void {
        const options = this.options;
    ...
    checkMenuState(deltaX: number, deltaY?: number): void {
    ...
    initialize(): void {
        const options = Object.assign({}, this.defaults, this.options || {}) as NavDrawerOptions;
        this.options = options;
