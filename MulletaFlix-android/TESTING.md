...
| Player OSD (`:feature:player`) | Auto-ocultar enquanto reproduz (inclusive TV), manter controles na pausa e revelar via setas/OK no controle remoto | JVM + Compose instrumentado em TV |
| Busca (`:feature:search`) | Debounce, filtros por tipo incluindo livros, sugestões filtradas, histórico limitado, remoção individual de histórico, erro e retry; chip de livros acessível por toque no tablet e foco/seleção por controle remoto na TV | JVM + Compose instrumentado |
| Home & Descoberta (`:feature:home`) | Carregamento assíncrono, expiração de sessão, seções dinâmicas, retomar conteúdo, atalho acessível para solicitar mídia, validação do ano e prevenção de envio duplicado | JVM + Compose instrumentado |
| Feedback por título (`:feature:item-detail`) | Reporte acessível com ID do título, categoria e descrição; erros permitem nova tentativa e envios duplicados são bloqueados | JVM + Compose instrumentado |
| Biblioteca (`:feature:library`) | Filtros por gênero/ano/classificação indicativa, aplicar/cancelar/limpar, validação de ano, persistência, ordenação e paginação | JVM + Compose instrumentado em TV/tablet |
...
| UX por dispositivo | Testes instrumentados executados no AVD de telefone, tablet e Android TV; cenários de foco D-pad/centro limitados a TV, fluxos touch e acessibilidade executados no tablet; retomada da Home por D-pad/centro e alvo touch mínimo | Android instrumentado |
