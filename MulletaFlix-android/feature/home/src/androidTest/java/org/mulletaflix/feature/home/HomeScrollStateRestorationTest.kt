...
class HomeScrollStateRestorationTest {
    @get:Rule
    val composeRule = createComposeRule()

    @Test
    fun homePositionSurvivesSavedStateRestoration() {
        val restorationTester = StateRestorationTester(composeRule)
        val items = (0 until 60).toList()
        var state: androidx.compose.foundation.lazy.LazyListState? = null

        restorationTester.setContent {
            val listState = rememberHomeScrollState()
            state = listState
    ...
    @Test
    fun homeCarouselPositionSurvivesSavedStateRestoration() {
        val restorationTester = StateRestorationTester(composeRule)
        val items = (0 until 60).toList()
        var state: androidx.compose.foundation.lazy.LazyListState? = null

        restorationTester.setContent {
            val listState = rememberHomeCarouselScrollState()
            state = listState
