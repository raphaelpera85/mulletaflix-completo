...
esta rodada mantém o escopo restrito ao APK, sem alteração ou release do servidor.

Regra obrigatória de release APK: as notas de cada versão devem descrever fielmente as
melhorias e correções efetivamente incluídas naquele APK. Atualize as notas junto com o
release e valide-as contra o diff/testes; não reutilize texto genérico nem anuncie itens
que não foram entregues. As notas do APK não devem descrever mudanças exclusivas do servidor.

> Nota desta rodada: as evidências de build e testes foram coletadas em 24/09/2026.
> O servidor não foi alterado nesta conversa APK-only; nenhuma release remota foi publicada. Antes do bump mais recente, a versão anterior conferida foi `1.3.53` (versionCode 354), SHA-256 `0AFFDEBDC32FA19A8964C659DD10552309E6550A716C1E713829234000B7B505`.

> Rodada APK v1.3.55: além da correção já registrada do SyncPlay (latest-wins, revalidação de sala/mídia/geração e invalidação por atualização da fila), os downloads offline usam diretório privado persistente fora de `cacheDir`; arquivos legados migram/mesclam sem sobrescrita, conflitos mantêm as duas cópias e a pasta antiga ativa. O player lê cache de downloads sem gravar streams não baixados. Teste instrumentado executa o `CacheDataSource` real e comprova cache-hit sem leitura upstream e reprodução upstream sem escrita no cache. Android TV: 20 testes instrumentados, 19 aprovados, 1 ignorado preexistente para touch target não aplicável à TV, 0 falhas. O gate final (`testDebugUnitTest`: 980 testes, 0 falhas/ignorados; `:app:lintDebug`; `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`. Emulador encerrado automaticamente e confirmado sem dispositivos conectados. O APK v1.3.55 que já existia (SHA-256 `BDB61D2BF10650043C30E9A017B692FBFBAD22B28C010D9707E47A8ADF35B092`) segue preservado. Sem publicação remota; respeitada a cadência de releases do APK.

> Rodada v1.3.54: comandos e atualizações SyncPlay de grupos diferentes do grupo ativo são descartados; eventos de conexão/desconexão não são apresentados sem uma sala ativa. Testes unitários cobrem correspondência de grupo e ausência de sala. `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` terminaram com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.54.apk`, 7.372.095 bytes, SHA-256 `9AD33AB2132E8FBF32C0E7F55AB799DC1704B365920A2ED6C461614C6524EC9C`. A publicação remota não foi feita.

> Rodada v1.3.53: o efeito de Snackbar do SyncPlay foi isolado em componente testável e a Android TV confirmou 1/1 teste instrumentado, verificando que eventos repetidos com a mesma mensagem continuam visíveis. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.53.apk`, 7.372.099 bytes, SHA-256 `0AFFDEBDC32FA19A8964C659DD10552309E6550A716C1E713829234000B7B505`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.52: o SyncPlay agora exibe Snackbar para conexão, desconexão, comandos, atualização de sala e troca de mídia/fila; cada evento incrementa uma sequência própria para que eventos repetidos também sejam percebidos. O último evento continua visível nos controles da sala. Foi adicionado teste unitário das mensagens de tempo real. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.52.apk`, 7.372.099 bytes, SHA-256 `1E01A68D925F1D0DD2C185C592FBE2CDC07D52EAE4E3F0D3295FA6368BEDAD99`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.51: a descoberta LAN agora invalida usuários e Quick Connect do endpoint público assim que seleciona um endereço local, e o carregamento persistido do DuckDNS não dispara quando a LAN já está ativa; isso evita misturar estados antes da verificação. Foi adicionado teste unitário da transição público → LAN. A Android TV executou 7/7 testes instrumentados de autenticação aprovados, sem skips ou falhas; o emulador foi encerrado pelo wrapper. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.51.apk`, 7.369.451 bytes, SHA-256 `A7F9D2B526420D15757ACC62DD31622B67EE9A75A276874011A5601EBCF56461`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.50: o retry de disponibilidade do Quick Connect ganhou cobertura unitária no `AuthViewModel`, verificando a sequência falha de `QuickConnect/Enabled` → erro acionável → retry → disponibilidade confirmada. A cobertura instrumentada da Android TV permanece validando os estados visuais. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.50.apk`, 7.369.451 bytes, SHA-256 `535524D2DAC96442DBE6B7441E88FE69BAA40B51E6ECF47611723BC9DAACAFA9`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.49: falhas de timeout/rede na consulta de disponibilidade do Quick Connect agora deixam de prender a tela em carregamento; a UI mostra uma mensagem acionável e `Tentar novamente`, que repete a consulta no servidor selecionado. Foram adicionados testes instrumentados para erro e retry, preservando a cobertura dos estados disponível e desconhecido. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 7/7 testes instrumentados de autenticação aprovados, sem skips ou falhas; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.49.apk`, 7.369.451 bytes, SHA-256 `A7596996CF623877C54A2581AAC5DD694BFA94AEA96CB4CE61E0E19111713E85`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.48: o Quick Connect agora aguarda a resposta de `QuickConnect/Enabled` antes de oferecer a geração do código; enquanto isso, a tela mostra `Verificando Quick Connect…`, evitando iniciar chamadas prematuras que pareciam falhas do recurso. Foi adicionado teste instrumentado para o estado desconhecido, mantendo a cobertura do início autorizado. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 6/6 testes instrumentados de autenticação aprovados, sem skips ou falhas; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.48.apk`, 7.369.451 bytes, SHA-256 `A14D76317B408B4A60269F65B1B1DE4DD7B87B0795DEF086C4BC45193FF5357E`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.47: atualizações de fila SyncPlay com drift de até 750 ms não forçam mais seek, reduzindo microtravadas sem perder a correção de divergências significativas; comandos explícitos continuam autoritativos. O teste unitário cobre drift pequeno, drift significativo e posição negativa. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 15/15 testes instrumentados aprovados na Biblioteca e 0 no módulo SyncPlay; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.47.apk`, 7.369.451 bytes, SHA-256 `592D64DFD0C04DC0BD8E5B8CBC036ED5AAB28FCAD2D3E4CAFC61CA939C630E46`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.46: o player passou a exibir um aviso não bloqueante enquanto o SyncPlay está desconectado e reconectando; o estado não aparece em mídia offline ou quando a sala está conectada. Foram adicionados teste unitário das mensagens e validação de layout na TV. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 15/15 testes instrumentados aprovados na Biblioteca e 0 no módulo SyncPlay; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.46.apk`, 7.369.451 bytes, SHA-256 `3657713368B1447CC46854929F2BA9A93913BCC99532841ECCA62E2BEA4BAC30`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.45: o cliente SyncPlay agora descarta callbacks de sockets substituídos durante reconexão ou troca de sala, incluindo mensagens que anteriormente poderiam controlar o player a partir de uma conexão antiga. Foi adicionado teste unitário da política de identidade do socket. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 15/15 testes instrumentados aprovados na Biblioteca e 0 no módulo SyncPlay; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.45.apk`, 7.369.451 bytes, SHA-256 `4B2A2AD74A36059423C69E56FA4680ABD5D1D7EFFEC76F2EA1464CEA0FBEA113`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.44: `Pause` e `Unpause` agora aplicam a posição autoritativa `PositionTicks` enviada pelo servidor antes de pausar ou retomar, enquanto `Stop` não executa seek implícito. O teste unitário da política foi ampliado para proteger esse contrato. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 15/15 testes instrumentados aprovados na Biblioteca e 0 no módulo SyncPlay; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.44.apk`, 7.369.451 bytes, SHA-256 `71192BE76FB02E4EA3541E4FDDE52DCB2EF8815FBD7AB4EB692368775E354DD6`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.43: o WebSocket SyncPlay agora reconecta automaticamente após falha ou fechamento inesperado, com backoff limitado de 1/2/4/8 segundos; sair da sala cancela a reconexão e invalida conexões antigas. Foram adicionados testes unitários da política de backoff. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 15/15 testes instrumentados aprovados na Biblioteca e 0 no módulo SyncPlay; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.43.apk`, 7.369.451 bytes, SHA-256 `CC13834674027F4D3313D649FCAEA986B6244C22AF0CCE0FCECDA3BB265ADED9`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.42: o APK passou a aplicar no player local os comandos remotos de SyncPlay (`Pause`, `Unpause`, `Stop` e `Seek`) e as atualizações de fila, incluindo troca de mídia, posição e estado de reprodução. O contrato foi alinhado aos campos reais do servidor (`GroupId`, `PlaylistItemId`, `ItemId`, `PositionTicks`, `When`), com descarte seguro de eventos de outra sala/mídia. Foram adicionados testes do parser e da política de filtragem/posição. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`; a Android TV executou 15/15 testes instrumentados aprovados na Biblioteca e 0 no módulo SyncPlay; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.42.apk`, 7.369.451 bytes, SHA-256 `FBB62AF1042548A8C7176B780970597F2147C43D607208257700EC8B37B3482A`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.41: o APK ganhou um cliente WebSocket autenticado para o endpoint `/socket` do servidor, com leitura de `SyncPlayCommand` e `SyncPlayGroupUpdate`, ciclo de vida ligado à sala e indicação visual de conexão/último evento. Foram adicionados testes unitários do parser; o gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) terminou com `BUILD SUCCESSFUL`. A Android TV executou 15 testes instrumentados, com 15 aprovados, 0 falhas e 0 ignorados; o emulador foi encerrado pelo wrapper. APK local: `dist/mulletaflix-app-v1.3.41.apk`, 7.353.067 bytes, SHA-256 `BB29FA73BBA9D1AEE38D512F148CC087A3165F17CA0570EB680FFD0E5C35D54F`.

> Rodada v1.3.40: SyncPlay agora expõe no APK os comandos oficiais de Pausar, Retomar e Parar reprodução (`SyncPlay/Pause`, `SyncPlay/Unpause`, `SyncPlay/Stop`) na sala ativa, com proteção contra envio duplicado e foco compatível com D-pad. Foram adicionados testes de contrato da API e teste unitário de submissão única. A suíte instrumentada da Biblioteca passou com 15/15 na Android TV; o módulo SyncPlay não possui testes instrumentados e reportou 0 testes. O gate completo (`testDebugUnitTest`, `:app:lintDebug`, `:app:assembleRelease`) e o empacotamento terminaram com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.40.apk`, 7.353.067 bytes, SHA-256 `F693CFC81D1D4128FD5CC45E13C149AA82436A026CDADF11D6F846BB01C16767`. Nenhuma release remota foi publicada; o escopo continua APK-only.

> Rodada v1.3.32: a troca de mídia agora encerra e aguarda a sessão remota anterior antes de iniciar a próxima, enquanto relatórios atrasados ficam invalidados por geração, sessão e usuário. O fluxo offline continua sem chamadas remotas indevidas. Foram executados 19 testes instrumentados do player na Android TV (18 aprovados, 1 skipped esperado, 0 falhas), além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.32.apk`, 7.353.067 bytes, SHA-256 `60F9A6A831DB3A27544CCB127BA68C6037D877CBB9915795C66DEBC5C41B5F78`.

> Rodada v1.3.33: falhas de conexão, login, cadastro e Quick Connect agora são convertidas em mensagens acionáveis para HTTP, timeout, host indisponível e bloqueio de cleartext, sem expor mensagens técnicas do Android/OkHttp. O endpoint real do servidor confirmou `QuickConnect/Enabled=true` e `QuickConnect/Initiate` respondeu 200 durante a auditoria. Foram executados 5 testes instrumentados de autenticação na Android TV (5 aprovados, 0 skipped, 0 falhas), além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.33.apk`, 7.336.683 bytes, SHA-256 `D08648413EE7BF87EA3215594F8F23691DE4CEBD7F2AC6619532B31061D0117E`.

> Rodada v1.3.34: os loops de atualização automática da Home e da Biblioteca na Android TV agora sobrevivem a falhas transitórias de um refresh; o cancelamento normal de lifecycle continua sendo propagado. Foram executados os testes instrumentados da Home (12/12) e Biblioteca (14/14), sem skips ou falhas, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.34.apk`, 7.336.679 bytes, SHA-256 `46E34B8C6BFC73FD41B9A0ED0CB42862AD1F9D231B13643C2CEC2553FFAF62B4`.

> Rodada v1.3.35: os timers de atualização de canais e EPG da TV ao vivo foram centralizados em um scheduler resiliente; falhas transitórias não encerram o polling e o lifecycle continua cancelando o trabalho fora do foreground. Foram executados 8 testes instrumentados do Live TV, sem skips ou falhas, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.35.apk`, 7.353.067 bytes, SHA-256 `9F87A81CE26E38933C07851AC14F49E7F0CDB28A409653783C694C91353AC5AF`.

> Rodada v1.3.36: Minha Lista/Favoritos agora reutiliza o scheduler resiliente da Biblioteca para atualização automática na Android TV; uma falha transitória não encerra o polling e o lifecycle continua cancelando o trabalho fora do foreground. A suíte instrumentada da Biblioteca passou com 14/14 testes, sem skips ou falhas; `testDebugUnitTest`, lint e `assembleRelease` também terminaram com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.36.apk`, 7.353.067 bytes, SHA-256 `2AEF6EADBD1109FBA95BDA678CFF89B7A8DAC4F10169FA22046F152ED1D0BC8C`.

> Rodada v1.3.37: Minha Lista agora observa o monitor de conectividade e reconcilia automaticamente os favoritos quando a rede retorna após uma interrupção, sem cancelar ou duplicar uma carga em andamento. Foi adicionado teste unitário de regressão; a suíte instrumentada da Biblioteca passou com 14/14 testes, sem skips ou falhas, e o gate completo da versão anterior passou com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.37.apk`, 7.353.067 bytes, SHA-256 `CC613BFFDD9E63F65AF8E9BDF0C861C4700EA0D804CDFF2A9A25858F44A681DC`.

> Rodada v1.3.38: Minha Lista passou a exibir o estado offline com banner contextual e retry focável, reutilizando o componente da Biblioteca para uma experiência consistente em TV/D-pad. O teste unitário cobre a indicação offline e sua limpeza após a reconexão; a suíte instrumentada da Biblioteca passou com 14/14 testes, sem skips ou falhas. APK local: `dist/mulletaflix-app-v1.3.38.apk`, 7.353.067 bytes, SHA-256 `FAAAEDD038CF40EC7B35C1EFF3A722978A59156EFEE68E2CEC7A0F020B3442F9`.

> Rodada v1.3.39: o banner offline compartilhado ganhou cobertura instrumentada explícita para a mensagem contextual de Minha Lista, incluindo visibilidade da ação `Tentar novamente` para a UX remota. A suíte instrumentada da Biblioteca passou com 15/15 testes, sem skips ou falhas; o gate completo também terminou com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.39.apk`, 7.353.067 bytes, SHA-256 `3095F6E29CB528773250E0484636E73AD774281E7B0334C40C153B7CED08FD31`.

> Rodada v1.3.14: o refresh de foreground da Android TV agora respeita o `Job` do carregamento inicial, evitando que a entrada na biblioteca cancele a primeira requisição. Testes unitários, lint, assembleRelease e 12 testes instrumentados da biblioteca na TV passaram. APK local: `dist/mulletaflix-app-v1.3.14.apk`, SHA-256 `E0A0948A1ABC52F7215141FFAFCA5400CE7F0CB9B42E22CFFD491188E114C6EE`.

> Rodada v1.3.15: a mesma proteção foi aplicada ao refresh da Home na Android TV. Testes unitários, lint, assembleRelease e 9 testes instrumentados da Home na TV passaram. APK local: `dist/mulletaflix-app-v1.3.15.apk`, SHA-256 `920D5AED5E35A5332A245C0BC5557FBF3ACC3C63252BB95E6972B2BB5D357007`.

> Rodada v1.3.16: a Home ganhou ação visível `Atualizar Home` no topo, acessível pelo D-pad/controle remoto, com estado ocupado preservando o alvo de foco. Testes unitários (456 tarefas acionáveis), lint, assembleRelease e 4 testes instrumentados focados na barra superior da Home na TV passaram. APK local: `dist/mulletaflix-app-v1.3.16.apk`, SHA-256 `1688B429DD514206B7211839A46A46EF04270003B73CE17FF33E7C13A4634A95`.

> Rodada v1.3.17: a conexão inicial deixou de anunciar a versão histórica fixa do servidor; a versão agora aparece somente após o handshake verificar o endpoint. O servidor ativo consultado anunciou `12.0.78`. Testes unitários (456 tarefas acionáveis), lint, assembleRelease e 3 testes instrumentados de autenticação na Android TV passaram. APK local: `dist/mulletaflix-app-v1.3.17.apk`, SHA-256 `EB45A947A22C4CC2680E5DC1CD49FD353A330F59F22A090FA42D9A607DD9E057`.

> Validação adicional pós-release: `MulletaflixTabletApi35` (API 35) executou 4 testes instrumentados de layout adaptativo/filtros da biblioteca; 4/4 passaram. O wrapper reportou erro apenas ao tentar enviar `emu kill` após a conexão do AVD já ter sido encerrada; a tarefa Gradle terminou com `BUILD SUCCESSFUL`.

> Rodada v1.3.18: o refresh automático de TV ao vivo agora respeita o `Job` ativo antes da publicação de `isLoading`, evitando que a entrada/foreground da Android TV cancele a primeira carga de canais. Testes unitários (456 tarefas acionáveis), lint e assembleRelease passaram; a regressão foi coberta no `LiveTvViewModelTest`. APK local: `dist/mulletaflix-app-v1.3.18.apk`, SHA-256 `39036C08A04E2426F1B36902E62DD85322B8B64B8D43574678230701BE6E7EA5`.

> Rodada v1.3.19: a barra superior da TV ao vivo foi extraída para um componente testável, preservando a navegação por D-pad entre voltar, atualizar e EPG e mantendo o EPG indisponível sem canais carregados. Foram executados 3 testes instrumentados da barra na `MulletaflixTvApi34`, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.19.apk`, SHA-256 `6FE0D67C4604A24896400D1A804889426AAE8F7FF1D3531B44B303AA304FF521`.

> Rodada v1.3.20: o diálogo do EPG agora atualiza automaticamente a programação a cada minuto enquanto permanece aberto e em foreground; fechado ou em background, não há polling. A política foi coberta em teste unitário, os 3 testes instrumentados da barra continuaram passando na Android TV, e `testDebugUnitTest`, lint e `assembleRelease` terminaram com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.20.apk`, SHA-256 `EC20D76C62B5F7E4E99A29CA9FD9E4E950425F322368F899C5928ED8F12D5B4B`.

> Rodada v1.3.21: o conteúdo visual do EPG agora possui cobertura instrumentada de Compose para o botão `Gravar`, o estado `Agendado` e a preservação dos programas junto ao retry após erro. Foram executados 6 testes instrumentados combinados na Android TV (guia e barra superior), além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.21.apk`, SHA-256 `2DF300307A9D49BA4D91C1D64C8747D562B769D9746DB0DA03EFB38EAD4ED117`.

> Rodada v1.3.22: as ações `Gravar`/`Agendado` do EPG agora exibem destaque vermelho quando recebem foco na Android TV, mantendo o comportamento de toque em celular e tablet. O wrapper de emulador também passou a tratar o encerramento já concluído do AVD como limpeza idempotente, evitando falso-negativo após testes bem-sucedidos. Foram executados 6 testes instrumentados na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.22.apk`, SHA-256 `882E20F8F5ED6324F84A3229CE66D9EBAA38E65F119F13796438FAC12B2E01E2`.

> Rodada v1.3.23: o destaque vermelho de foco remoto do EPG foi unificado para `Gravar`, `Agendado`, retry de erro e `Fechar`, mantendo o comportamento de toque em celular e tablet. Foram executados 6 testes instrumentados na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.23.apk`, SHA-256 `603E58804BEC29B8DD5B5B12759FEAA4AD6AC33B3E7EEC44C3E69C90D9225613`.

> Validação adicional pós-v1.3.23: o conteúdo do EPG foi executado no `MulletaflixTabletApi35` (Android 15), com 3/3 testes instrumentados aprovados. Nenhuma alteração de código ou nova versão foi gerada nesta validação.

> Rodada v1.3.24: o cartão de erro do player foi extraído para um componente testável e o botão `Tentar novamente` agora exibe destaque vermelho quando recebe foco na Android TV, preservando o retry para streams online e reprodução offline. Foram executados 4 testes instrumentados do player na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.24.apk`, SHA-256 `19B32C6BCAF210F4C659CB4BAB39875E24CE30182EE5E2FFD28CA1FFC1052625`.

> Rodada v1.3.26: o prompt de “Próximo Episódio” agora direciona o foco para `Assistir Agora` na Android TV e destaca a ação principal em vermelho, mantendo `Cancelar` e o comportamento de toque em outras superfícies. Foram executados 5 testes instrumentados do player na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.26.apk`, SHA-256 `06BD01965DBF706FFE4DCD30BD6199CE11D53FBB22A88FFB1563BC2984B43F8D`.

> Rodada v1.3.27: os retries de erro dos canais e das gravações da TV ao vivo agora usam o mesmo destaque vermelho de foco remoto do EPG, mantendo o retry por toque em celular/tablet e a atualização existente. Foram executados 6 testes instrumentados da TV ao vivo na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.27.apk`, SHA-256 `61B7559C69F2FD6C39F78B2C8ADB011EE93E6C7C224E96CC5D84B8F2AA4CE0C8`.

> Rodada v1.3.28: os três cartões de erro recuperáveis da Home agora destacam em vermelho o retry quando ele recebe foco na Android TV, mantendo o comportamento de toque em celular/tablet. Foram executados 5 testes instrumentados da Home na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.28.apk`, SHA-256 `5D157CC6383E40F6323F426C6DDC761E458F7ADBDCA84C74A61E8BB7FDEC88E0`.

> Rodada v1.3.29: as opções de áudio, legendas e qualidade do player agora exibem destaque vermelho quando recebem foco pelo controle remoto na Android TV, preservando seleção por toque e semântica de rádio em outras superfícies. Foram executados 12 testes instrumentados do player na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.29.apk`, SHA-256 `D6F4A4CA8C047A0326670E2FEA357D4DC6419A8FE2490F0D28E8A14E69FBF317`.

> Rodada v1.3.30: os menus de velocidade, temporizador e proporção da tela agora usam o mesmo destaque vermelho de foco remoto na Android TV, mantendo a seleção da linha inteira e a semântica de rádio. Foram executados 8 testes instrumentados do player na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.30.apk`, SHA-256 `5E4F842BF44F4DE67FBDDBDD228B80091C599CF40A1C52D82627DBBFBECFA4E8`.

> Rodada v1.3.31: os retries de erro inicial, erro em grade e banner offline da biblioteca agora exibem o destaque vermelho de foco remoto na Android TV, mantendo a grade adaptativa e o comportamento de toque. Foram executados 5 testes instrumentados da biblioteca na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.31.apk`, 7.353.067 bytes, SHA-256 `8BDF480C8BEAC44DE5951E6066D9C00B34E07E175559CBF9B3EBBF8798671385`.

> Rodada v1.3.25: quando um erro de reprodução aparece na Android TV, o foco agora é direcionado automaticamente para `Tentar novamente`, mantendo o comportamento sem captura automática em celular e tablet. Foram executados 4 testes instrumentados do player na Android TV, além de `testDebugUnitTest`, lint e `assembleRelease`, todos com `BUILD SUCCESSFUL`. APK local: `dist/mulletaflix-app-v1.3.25.apk`, SHA-256 `E9209C951401541FAE90408C08579BDEA82A8D86C0D98FA2913F5DF18F2CB75A`.
...
- Versão anterior conferida antes do bump: **1.3.38**, `versionCode=339`, SHA-256 `FAAAEDD038CF40EC7B35C1EFF3A722978A59156EFEE68E2CEC7A0F020B3442F9`. APK local atual validado: **1.3.39**, `versionCode=340`, SHA-256 `3095F6E29CB528773250E0484636E73AD774281E7B0334C40C153B7CED08FD31`.
...
- Quality Bar da rodada v1.3.33: `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` passaram com código 0; `:feature:auth:connectedDebugAndroidTest` passou com 5 testes, 0 skipped e 0 falhas no AVD `MulletaflixTvApi34`; APK recompilado e empacotado com `versionName=1.3.33`/`versionCode=334`; o endpoint real confirmou Quick Connect habilitado e a auditoria não alterou o servidor. O servidor não foi alterado nem publicado nesta conversa APK-only.
- Quality Bar da rodada v1.3.34: `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` passaram com código 0; as suítes instrumentadas Home (12 testes) e Biblioteca (14 testes) passaram no AVD `MulletaflixTvApi34`; a correção mantém o scheduler vivo após falha transitória e relança cancelamento de lifecycle. O APK foi empacotado localmente e o servidor não foi alterado nem publicado nesta conversa APK-only.
- Quality Bar da rodada v1.3.35 antes do empacotamento: `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` passaram com código 0; a suíte instrumentada do Live TV passou com 8 testes no AVD `MulletaflixTvApi34`; o scheduler de canais/EPG mantém falhas isoladas sem quebrar o foreground loop. O servidor não foi alterado nem publicado nesta conversa APK-only.
- Quality Bar da rodada v1.3.35: o APK foi empacotado localmente com o SHA-256 registrado acima; o servidor não foi alterado nem publicado nesta conversa APK-only.
- Quality Bar da rodada v1.3.36 antes do empacotamento: `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` passaram com código 0; a suíte instrumentada da Biblioteca passou com 14/14 no AVD `MulletaflixTvApi34`; o scheduler de Minha Lista/Favoritos reutiliza o tratamento resiliente de falhas da Biblioteca. O servidor não foi alterado nem publicado nesta conversa APK-only.
- Quality Bar da rodada v1.3.36: o APK foi empacotado localmente com o SHA-256 registrado acima; nenhum release do servidor foi executado ou publicado.
- Quality Bar da rodada v1.3.37 antes do empacotamento: `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` passaram com código 0; a suíte instrumentada da Biblioteca passou com 14/14 no AVD `MulletaflixTvApi34`; a recuperação de Minha Lista no retorno da rede foi coberta por teste unitário. O servidor não foi alterado nem publicado nesta conversa APK-only.
- Quality Bar da rodada v1.3.38 antes do empacotamento: `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` passaram com código 0; a suíte instrumentada da Biblioteca passou com 14/14 no AVD `MulletaflixTvApi34`; o banner offline contextual e o retry foram cobertos pelo fluxo unitário de reconexão. O servidor não foi alterado nem publicado nesta conversa APK-only.
- Quality Bar da rodada v1.3.39 antes do empacotamento: `testDebugUnitTest`, `:app:lintDebug` e `:app:assembleRelease` passaram com código 0; a suíte instrumentada da Biblioteca passou com 15/15 no AVD `MulletaflixTvApi34`; o caso contextual de Minha Lista foi validado com Compose instrumentado. O servidor não foi alterado nem publicado nesta conversa APK-only.
